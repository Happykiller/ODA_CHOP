...
SlideDeck.prototype.onBodyKeyDown_ = function(e) {
  ...
  switch (e.keyCode) {
    ...
    case 40: // down arrow
        if(g_nextSlide != null){
            $('#div_content').html('<IMG SRC=\"API/img/loading.gif\" ALT=\"Chargement\" TITLE=\"Chargement\">');
            var params = {
                lang:null
                ,scenario:g_nextSlide.scenario
                ,module:g_nextSlide.module
                ,page:g_nextSlide.page
    ...
    case 38: // up arrow
        if(g_prevSlide != null){
            ...
            var params = {
                lang:null
                ,scenario:g_prevSlide.scenario
                ,module:g_prevSlide.module
                ,page:g_prevSlide.page
                ,milis:$.functionsLib.getMilise()
            };
            $.functionsStorage.set('INTERCOM_CHOP_MSG-TO-PRESS',params,60);
...
SlideDeck.prototype.loadConfig_ = function(config) {
  ...
  if (this.config_.presenters) {
    ...
    if (presenters.length == 1) {
      ...
      var email = p.email ? tabElt.push('<span>email</span>'+
          '<a href="mailto:' + p.email + '">' +
          p.email + '</a>') : '';

      var gplus = p.gplus ? tabElt.push('<span>g+</span><a href="' + p.gplus +
          '">' + p.gplus.replace(/https?:\/\//, '') + '</a>') : '';

      var twitter = p.twitter ? tabElt.push('<span>twitter</span>' +
          '<a href="http://twitter.com/' + p.twitter + '">' +
          p.twitter + '</a>') : '';

      var www = p.www ? tabElt.push('<span>www</span><a href="' + p.www +
                        '">' + p.www.replace(/https?:\/\//, '') + '</a>') : '';

      var github = p.github ? tabElt.push('<span>github</span><a href="' + p.github +
          '">' + p.github.replace(/https?:\/\//, '') + '</a>') : '';

      var html2 = tabElt.join('<br>');
